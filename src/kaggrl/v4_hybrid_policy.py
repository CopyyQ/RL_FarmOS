...
from .v4_labor_optimizer import (
    optimize_idle_weed_labor,
    optimize_late_hires,
)
from .v4_farm_supervisor import (
    supervise_farm_care,
    optimize_safe_idle_farm_labor,
)
from .v4_structural_gate import structural_override_for_state
...
StructuralRouteFn = Callable[
    [dict[str, Any], Any, int, StepStrategyContext], Any
]
...
class FarmOSV4HybridPolicy:
    ...
    def __init__(
        ...
        *,
        ...
        structural_route_policy: StructuralRouteFn | None = None,
        ...
        self.structural_route_policy = structural_route_policy
        ...
        self.enable_idle_weed_labor = bool(enable_idle_weed_labor)
        self.enable_farm_care_supervisor = bool(enable_farm_care_supervisor)
        self.enable_safe_idle_labor = bool(enable_safe_idle_labor)
        self.enable_late_hire_pruning = bool(
            enable_late_hire_pruning
        )
        self.allow_all_routes = bool(allow_all_routes)
        self.enable_structural_route_gate = bool(
            enable_structural_route_gate
        )
        self.structural_route_id: int | None = None
        self.structural_until_step = -1
        self.structural_reason: str | None = None
    ...
    def act(self, observation, configuration=None) -> dict[str, Any]:
        ...
        compatible_routes = (
            ...
            else self.base.compatible_route_ids(
                obs, configuration,
            )
        )
        ...
        structural_route_before = int(selected_route)
        structural_override_applied = False
        structural_metadata = None

        # Exact-causal structural gate. It is deliberately independent from
        # the learned compatible-route mask: the point of this layer is to
        # choose a different economic structure when exact full-game evidence
        # says the static macro family is leaving money on the table.
            ...
            try:
                ...
                if self.structural_route_id is None:
                    ...
                    if decision is not None:
                        ...
                        self.structural_reason = str(decision.reason)
                        started = True
                ...
                if (
                    ...
                    selected_route = int(self.structural_route_id)
                    # Counterfactual replay preserves the learned market choice
                    # only on the switching step; while the structural option
                    # remains active it uses KEEP_ROUTE.
                    if not started:
                        market_mode = "KEEP_ROUTE"
                    action = self.base.action_for_route(
                        obs, selected_route, configuration,
                    ...
                    action = compile_market_mode(
                        obs, action, market_mode,
                    )
                    option_applied = True
                    structural_override_applied = (
                        int(selected_route) != structural_route_before
                    )
                    structural_metadata = {
                        "applied": True,
                        "started": bool(started),
                        "route_id": int(selected_route),
                        "until_step": int(self.structural_until_step),
        ...
        # External structural policies remain available for experiments, but
        # do not override an active exact-causal gate.
        if (
            self.structural_route_policy is not None
            and self.structural_route_id is None
        ):
            try:
                override = self.structural_route_policy(
                    obs,
                    configuration,
                    int(selected_route),
                    context,
        ...
        self.last_strategy = {
            ...
            "base_route_id": int(base_route_id),
            "actor_selected_route_id": int(structural_route_before),
            "selected_route_id": int(selected_route),
            "structural_override_applied": bool(
                structural_override_applied
            ),
            "structural_route_id": (
                int(selected_route)
                if structural_override_applied
                else None
            ),
            "structural_gate": structural_metadata,
        ...
        micro_metadata = None
        if self.option_policy is not None and hasattr(
            self.option_policy, "micro_action"
        ):
            try:
                action, micro_metadata = self.option_policy.micro_action(
                    obs,
                    configuration,
                    int(selected_route),
                    action,
                ...
                micro_metadata = {"applied": False, "error": True}

        idle_farm_metadata = None
        if self.enable_safe_idle_labor:
            try:
                turns_per_day = int(
                ...
                action, idle_farm = optimize_safe_idle_farm_labor(
                    obs,
                    action,
                    route=self.base.routes.get(int(selected_route)),
                    step=int(step),
                    turns_per_day=turns_per_day,
                )
                idle_farm_metadata = {
                    "applied": bool(idle_farm.applied),
                    "idle_hands": int(idle_farm.idle_hands),
                    "moved": int(idle_farm.moved),
                    "watered": int(idle_farm.watered),
                    "fed": int(idle_farm.fed),
                    "cared": int(idle_farm.cared),
                    "harvested": int(idle_farm.harvested),
                    "collected_fertilizer": int(idle_farm.collected_fertilizer),
                    "fertilized": int(idle_farm.fertilized),
                    "pickups": int(idle_farm.pickups),
                    "rescue_buys": int(idle_farm.rescue_buys),
                }
            except Exception:
                idle_farm_metadata = {"applied": False, "error": True}

        care_metadata = None
        try:
            if not self.enable_farm_care_supervisor:
                raise RuntimeError("farm care supervisor disabled")
            action, care = supervise_farm_care(
                obs,
                action,
                remaining_steps=int(clock.remaining_steps),
            )
            care_metadata = {
                "applied": bool(care.applied),
                "critical_plants": int(care.critical_plants),
                "critical_animals": int(care.critical_animals),
                "overrides": int(care.overrides),
                "water_orders": int(care.water_orders),
                "feed_orders": int(care.feed_orders),
                "care_orders": int(care.care_orders),
                "fertilize_orders": int(care.fertilize_orders),
                "wheat_rescue_buys": int(care.wheat_rescue_buys),
            }
        except Exception:
            care_metadata = {"applied": False, "error": True}

        labor_metadata = None
        if self.enable_idle_weed_labor:
            try:
                action, labor = optimize_idle_weed_labor(obs, action)
                labor_metadata = {
                    "applied": bool(labor.applied),
                    "weeds_dug": int(labor.weeds_dug),
                    "invalid_digs_removed": int(
                        labor.invalid_digs_removed
                    ),
                    "unit_changes": [
                        [int(idx), list(before), list(after)]
                        for idx, before, after in labor.unit_changes
                    ],
                }
            except Exception:
                labor_metadata = {
        ...
        hire_metadata = None
        if self.enable_late_hire_pruning:
            try:
                action, hire = optimize_late_hires(
                    obs,
                    action,
                    remaining_steps=int(clock.remaining_steps),
                    max_hires_today=9,
                ...
                hire_metadata = {
                    "applied": bool(hire.applied),
                    "hires_removed": int(hire.hires_removed),
                    "hires_today_before": int(
                        hire.hires_today_before
                    ),
                    "remaining_steps": int(hire.remaining_steps),
                }
            except Exception:
                hire_metadata = {
                    "applied": False,
        ...
        if self.last_strategy is not None:
            self.last_strategy["micro_policy"] = micro_metadata
            self.last_strategy["idle_farm_labor"] = idle_farm_metadata
            self.last_strategy["farm_care"] = care_metadata
            self.last_strategy["labor"] = labor_metadata
            self.last_strategy["hire_pruning"] = hire_metadata
