...
def optimize_sell_order(
    ...
    *,
    ...
    # Score the actual slot order with the same lockstep price dynamics as the
    # reference market. This directly captures the important first-mover effect:
    # selling a price-sensitive/high-value product earlier can preserve revenue
    # before either side increases market inventory and pushes the quote down.
    best_market = [list(order) for order in market]
    _, _, baseline_score = simulate_lockstep_sell_margin(
        observation,
        market,
        opponent,
    )
    ...
    for start, end in blocks:
        block = [list(order) for order in best_market[start:end]]
        width = end - start

        if width <= max(2, int(max_exact_sells)):
            # Exact permutation search for small SELL blocks. The whole market
            # tape is scored for each candidate, so interactions with rival SELL
            # slots and earlier/later own SELLs are included.
            seen = set()
            local_best = [list(order) for order in block]
            local_best_score = best_score
            for perm in itertools.permutations(block):
                signature = tuple(tuple(order) for order in perm)
                if signature in seen:
                    continue
                seen.add(signature)
                candidate = [list(order) for order in best_market]
                candidate[start:end] = [list(order) for order in perm]
                _, _, score = simulate_lockstep_sell_margin(
                    observation,
                    candidate,
                    opponent,
                )
                if score > local_best_score + 1e-9:
                    ...
                    local_best = [list(order) for order in perm]
            if local_best != block:
                best_market[start:end] = local_best
                best_score = float(local_best_score)
            continue

        # Fallback for wider blocks: rank by quote exposure, but still accept the
        # heuristic reorder only when exact lockstep simulation says it improves
        # expected margin.
        priorities = [
        ...
        if ranked_block != block:
            candidate = [list(order) for order in best_market]
            candidate[start:end] = ranked_block
            _, _, candidate_score = simulate_lockstep_sell_margin(
                observation,
                candidate,
                opponent,
            )
    ...
    gain = float(best_score) - float(baseline_score)
    applied = bool(
...
def front_run_future_sales(
    ...
    *,
    ...
    race_tracker: MarketRaceTracker | None = None,
    ...
    # Estimate how much rival supply can hit each product before our
    # scheduled future sale. Front-running is useful only when selling now
    # protects quote value that is likely to disappear after rival supply.
    predicted_orders = (
        race_tracker.predicted_opponent_orders(observation)
        if race_tracker is not None
        else []
    )
    predicted_rival_units = {item: 0 for item in PRODUCTS}
    for raw in predicted_orders:
        parsed = _sell(raw)
        if parsed is not None:
            item, quantity = parsed
            predicted_rival_units[item] += int(quantity)

    market_inventory = market_obs.get("inventory") or {}
    market_params = market_obs.get("params")
    candidates: list[
        tuple[float, str, int, list[tuple[int, int]]]
    ] = []
    ...
    for item in PRODUCTS:
        ...
        rival_units = max(0, int(predicted_rival_units.get(item, 0)))
        if rival_units <= 0:
            continue
        inventory = int(
            market_inventory.get(item, MARKET_I0) or MARKET_I0
        )
        now_revenue = sum(
            market_price(item, inventory + offset, market_params)
            for offset in range(quantity)
        )
        delayed_revenue = sum(
            market_price(
                item,
                inventory + rival_units + offset,
                market_params,
            )
            for offset in range(quantity)
        )
        expected_quote_loss = float(now_revenue - delayed_revenue)
        if expected_quote_loss <= 0.0:
            continue
        candidates.append(
            (expected_quote_loss, item, quantity, reservations)
        )

    # Pull forward the sales with the largest expected quote loss first.
    # The downstream exact sell-order optimizer then handles slot ordering
    # between all sales already present in the current action.
    debts: dict[int, dict[str, int]] = {}
    for expected_quote_loss, item, quantity, reservations in sorted(
        candidates,
