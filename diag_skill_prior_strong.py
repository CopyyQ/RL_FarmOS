from pathlib import Path
import copy, math
import numpy as np
import torch
from winner_train import exact_games, skill_teacher_records_to_batch, skill_bc_update
from v45_skill_runtime import V45SkillRuntime
from kaggrl.v4_farm_supervisor import MICRO_TASKS

ROOT=Path(__file__).resolve().parent
PARENT=ROOT/"assets/parent_promoted_v2.pt"; SNAP=ROOT/"runs/v46_econ_shadow/winner_v4_latest.pt"; OPP=ROOT/"assets/v51_main.py"
VAL_SEEDS=[26092407,26092410,26092417,26092420,26092427,26092430]
NATURAL=np.asarray([1950,36410,12170,9663,9756,2849,14386,2374,4083,773,6690,1440],dtype=np.float32)
POWERS=(1.15,1.30,1.50,1.80)

def make_actor_optimizer(device,checkpoint):
    rt=V45SkillRuntime(str(PARENT),str(SNAP),stochastic=False,temperature=1.0,residual_scale=1.0,
        base_keep_bias=2.3,decision_every=24,allowed_market_modes=None,seed=123,skill_cutover_step=720,
        skill_keep_penalty=2.75,skill_shadow_start_step=0,skill_confidence_threshold=0.70)
    actor=rt.base.actor.to(device)
    prefixes=("micro_trunk.","micro_task.","micro_value.")
    params=[p for n,p in actor.named_parameters() if n.startswith(prefixes)]
    opt=torch.optim.AdamW(params,lr=0.000125,weight_decay=1e-5)
    if checkpoint.get("micro_optimizer_state"):
        opt.load_state_dict(copy.deepcopy(checkpoint["micro_optimizer_state"]))
        for g in opt.param_groups:g["lr"]=0.000125
    return actor,opt

def set_prior_weights(batch,power):
    target=batch["target"]
    counts=torch.bincount(target,minlength=len(MICRO_TASKS)).detach().cpu().numpy().astype(np.float32)
    nz=counts[1:][counts[1:]>0]; ref=float(np.median(nz))
    nn=NATURAL[1:][NATURAL[1:]>0]; nref=float(np.median(nn))
    cw=np.ones(len(MICRO_TASKS),dtype=np.float32); cw[0]=0.05
    for i in range(1,len(MICRO_TASKS)):
        if counts[i]<=0: continue
        rb=math.sqrt(ref/max(1.0,float(counts[i])))
        pr=(float(NATURAL[i])/max(1.0,nref))**float(power) if NATURAL[i]>0 else 1.0
        cw[i]=2.0*max(0.25,min(4.0,rb*pr))
    batch["weight"]=torch.as_tensor(cw,dtype=torch.float32,device=target.device)[target]
    return cw

def main():
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    rows=exact_games(PARENT,SNAP,OPP,VAL_SEEDS,6,False,1.0,1.0,2.3,24,1056,both_seats=False,
        skill_cutover_step=720,skill_keep_penalty=2.75,skill_shadow_start_step=0,skill_confidence_threshold=0.70)
    vals=[r for g in rows for r in (g.get("skill_teacher_records") or [])]
    replay=torch.load("/tmp/v46_prior_ab_teacher.pt",map_location="cpu",weights_only=False)["records"]
    dims=torch.load(PARENT,map_location="cpu",weights_only=False); hd=int(dims["hidden_dim"]);cd=int(dims["clock_dim"])
    vb=skill_teacher_records_to_batch([{"skill_teacher_records":vals}],hd,cd,device)
    ck=torch.load(SNAP,map_location="cpu",weights_only=False)
    wi=MICRO_TASKS.index("WATER");fi=MICRO_TASKS.index("FERTILIZE");di=MICRO_TASKS.index("DIG_WEED");dpi=MICRO_TASKS.index("DEPLOY_ANIMAL")
    for pw in POWERS:
        actor,opt=make_actor_optimizer(device,ck)
        tb=skill_teacher_records_to_batch([{"skill_teacher_records":replay}],hd,cd,device)
        cw=set_prior_weights(tb,pw)
        torch.manual_seed(456789);
        if torch.cuda.is_available(): torch.cuda.manual_seed_all(456789)
        skill_bc_update(actor,tb,opt,temperature=1.0,epochs=4,minibatch=512,keep_penalty=2.75,update=True)
        v=skill_bc_update(actor,vb,opt,temperature=1.0,epochs=0,minibatch=1024,keep_penalty=2.75,update=False)
        def met(i): return tuple(round(float(x),4) for x in (v["per_task_precision"][i],v["per_task_recall"][i],v["per_task_f1"][i]))
        print("RESULT",pw,"core_f1",round(v["core_min_f1"],4),"acc",round(v["acc"],4),"nonkeep",round(v["nonkeep_acc"],4),
              "WATER",met(wi),"FERT",met(fi),"DIG",met(di),"DEPLOY",met(dpi),
              "wW",round(float(cw[wi]),3),"wF",round(float(cw[fi]),3),flush=True)
        pairs=[]
        for t,row in enumerate(v["confusion"]):
            for p,c in enumerate(row):
                if t!=p and c:pairs.append((int(c),t,p))
        pairs.sort(reverse=True)
        print("CONF",pw,"/".join(f"{MICRO_TASKS[t]}->{MICRO_TASKS[p]}:{c}" for c,t,p in pairs[:5]),flush=True)

if __name__=="__main__": main()
